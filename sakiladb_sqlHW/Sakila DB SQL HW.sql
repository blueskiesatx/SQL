...
-- the type BLOB, as the difference between it and VARCHAR are significant).
 -- A blol is a collection of binary data.
ALTER TABLE actor
...
-- Possible approach if other one fails or larger DB:
-- UPDATE actor 
-- SET first_name = (
...
-- Used the tables 'staff' and 'address':
...
SELECT f.title AS 'Film', COUNT(fa.actor_id) AS 'Tot Actors in Film'
FROM film f
...
-- 6d How many copies of the film Hunchback Impossible exist in the inventory system? I MUST SEE THIS MOVIE
...
-- subqueries to display the titles of movies starting with the letters K and Q whose 
-- language is English. 
USE sakila;
